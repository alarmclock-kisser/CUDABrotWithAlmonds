...
extern "C" __global__ void onlyEdges02(
...
int backB)
{
    ...
    unsigned char backBclamped = (unsigned char)min(max(backR, 0), 255);  // R -> B
    unsigned char backGclamped = (unsigned char)min(max(backG, 0), 255);
    unsigned char backRclamped = (unsigned char)min(max(backB, 0), 255);  // B -> R

    int clampedThickness = max(0, min(thickness, 10));
    float absThreshold = fabsf(threshold);

    outputPixels[pixelPos + 0] = backRclamped;
    outputPixels[pixelPos + 1] = backGclamped;
    outputPixels[pixelPos + 2] = backBclamped;
        ...
        for (int dy = -1; dy <= 1; dy++) {
            for (int dx = -1; dx <= 1; dx++) {
                ...
                float3 rgb = make_float3(
                    ...
                    inputPixels[neighborPos + 0] / 255.0f
                );
